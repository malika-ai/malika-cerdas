...
export const Clients: React.FC = () => {
  ...
  const MarqueeRow = ({ items, reverse = false }: { items: string[], reverse?: boolean }) => (
    <div className="flex w-full overflow-hidden mb-8 mask-linear-fade relative">
      ...
      <div className={`flex gap-12 ${reverse ? 'animate-scroll-reverse' : 'animate-scroll'} hover:[animation-play-state:paused]`}>
        {/* Render twice for infinite loop */}
        {[...items, ...items].map((imgUrl, index) => (
          <div 
            key={index} 
            className="flex-shrink-0 w-[150px] h-[80px] flex items-center justify-center transition-all duration-300 opacity-60 hover:opacity-100 hover:scale-110"
          >
            <img 
              src={imgUrl} 
              alt="Client logo" 
  ...
  return (
    <Section id="clients" className="bg-transparent overflow-hidden">
